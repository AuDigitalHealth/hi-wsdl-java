...
**This line (`17.0.0`):** Java **17**, **Jakarta** XML WS / Bind, **26** HI B2B **`Service`** stubs (full MCA), **committed** generated types (no **`wsimport`** in the default build). Compile dependencies: **`jakarta.xml.bind-api`** **4.0.5** and **`jakarta.xml.ws-api`** **4.0.3**. Add Eclipse EE4J **`com.sun.xml.ws:jaxws-rt`** **4.0.5** at runtime in your application when you invoke SOAP endpoints. This JAR does not bundle **`jaxws-rt`**.
...
| ------------- | ------- | --------- | --------------- |
| **8.0.0** | **8** | **`javax.*`** / EE4J **`jaxws-rt` 2.3.x** at runtime in consumers | **14** (standard HI B2B) |
| **11.0.0** | **11** | **Jakarta** / EE4J **`jaxws-rt` 4.0.x** at runtime in consumers | **26** (full MCA) |
| **17.0.0** | **17** | **Jakarta** / EE4J **`jaxws-rt` 4.0.x** at runtime in consumers | **26** (full MCA) |
...
The **17.0.0** JAR ships **Jakarta** types and the full MCA binding set (**26** `Service` stubs). **8.0.0** is **`javax`** with **14** stubs (standard HI B2B only).

---

## Local development (SNAPSHOT)

This repository builds **`17.0.0-SNAPSHOT`**. The default lifecycle compiles **committed** types only. To make an unpublished JAR resolvable for other local projects:
...
If Maven warns that a **GA** POM is missing (for example **`17.0.0`** before Central publish), clear stale **`au/gov/nehta/hi-wsdl`** entries in your **local Maven repository** (folders with only **`.lastUpdated`** files) and reinstall the SNAPSHOT. **`mvn clean`** in this project does not clear the local repository cache.
...
Prerequisites: **JDK 17+**, **Maven 3.6+**. Generated Java is **committed**; the default build does not run **`wsimport`**.
...
| Codegen | **26** committed `Service` stubs; **`-Pregenerate-sources`** for maintainer refresh only | **`17.0.0`**: in-repo **`wsimport`** (26 executions) by default; optional **`-Phi-wsdl-artifact`** uses matching **`hi-wsdl`** from Maven Central |
