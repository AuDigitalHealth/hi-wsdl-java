...
**This line (`8.0.0`):** Java **8**, **`javax.xml.ws`** / **`javax.xml.bind`**, **14** standard HI B2B **`Service`** stubs, **committed** generated types (no **`wsimport`** in the build). Add Eclipse EE4J **`com.sun.xml.ws:jaxws-rt`** **2.3.7** at runtime in your application when you invoke SOAP endpoints. This JAR does not bundle **`jaxws-rt`**.
...
The **8.0.0** JAR ships the standard HI B2B binding set only (**14** `Service` stubs, **`javax`**). **11.0.0.1** and later lines use **Jakarta** and include **26** stubs (full MCA).
The current state of the **8.0.0** line is the original committed types, minimally updated to allow a successful build.
...
If Maven warns that a **GA** POM is missing (for example **`8.0.0`** before Central publish), clear stale **`au/gov/nehta/hi-wsdl`** entries in your **local Maven repository** (folders with only **`.lastUpdated`** files) and reinstall the SNAPSHOT. **`mvn clean`** in this project does not clear the local repository cache.
