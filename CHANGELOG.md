...
= 8.0.0 =
=======
- Maven **`au.gov.nehta:hi-wsdl`** **8.0.0** (Java **8** / **`javax`**, **14** `Service` stubs). The first number of the Maven version is the targeted Java SE version. Consumers that pin **`hi-wsdl`** to **`${project.version}`** (including **`hi-b2b-client`**) use the same coordinate.
